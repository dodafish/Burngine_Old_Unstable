...
void StereoConfig::SetHMDInfo(const HMDInfo& hmd) {
	...
	Distortion.SetChromaticAberration(	hmd.ChromaAbCorrection[0],
										hmd.ChromaAbCorrection[1],
										hmd.ChromaAbCorrection[2],
										hmd.ChromaAbCorrection[3]);
...
void StereoConfig::SetDistortionFitPointVP(	float x,
											float y) {
	DistortionFitX = x;
	DistortionFitY = y;
	DirtyFlag = true;
...
void StereoConfig::SetDistortionFitPointPixels(	float x,
												float y) {
...
const StereoEyeParams& StereoConfig::GetEyeRenderParams(StereoEye eye) {
	static const UByte eyeParamIndices[3] = {
	0,
	0,
	1 };
...
void StereoConfig::updateEyeParams() {
	// Projection matrix for the center eye, which the left/right
	...
	switch (Mode) {
		...
		case Stereo_LeftRight_Multipass: {
			Matrix4f projLeft = Matrix4f::Translation(ProjectionCenterOffset, 0, 0) * projCenter, projRight =
