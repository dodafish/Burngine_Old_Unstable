...
void StaticMeshNode::draw(Camera* cam) {
	...
	if(Window::isContextCreated()){
		...
		glm::mat4 MVP;
		if(cam != nullptr){
			Matrix4f Projection = glm::perspective(cam->getFov(),
					cam->getAspectRatio(), 0.1f, 100.0f);
			Matrix4f View = glm::lookAt(cam->getPosition(), cam->getLookAt(),
					glm::vec3(0, 1, 0));
		...
		if(_material.getType() == Material::SOLID_COLOR){
			...
			glUniformMatrix4fv(
					BurngineShaders::getShaderUniformLocation(
							BurngineShaders::SOLID_COLOR, MVP_UNIFORM), 1,
					GL_FALSE, &MVP[0][0]);
